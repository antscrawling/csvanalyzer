...
def generate_initial_data():
       ...
       # discount_period_name = is_discount_period(date, current_discount_periods)
        ...
        for receipt in range(num_receipts):
            ...
            if customer_id not in customer_ages:
                ...
                country = selected_city['country_id']
            ...
            for product_idx in selected_products:
                ...
                transactions.append({
                    'date': int(f'{transaction_datetime.year}{transaction_datetime.month}{transaction_datetime.day}{transaction_datetime.hour:02}{transaction_datetime.minute:02}'),
  ...
  # df['hour_cos'] = np.cos(2 * np.pi * df['hour'] / 24)
    ...
    with tqdm(total=4, desc="Database operations") as pbar:
        ...
        con.register('df_view', df_for_db)
        con.execute("CREATE TABLE sales_data AS SELECT * FROM df_view")
